...
barplot(height = 100 * prop.table(table(dados$`Grau de Instrução`)),
        xlab = "Grau de Instrução", ylab = "Porcentagem",
        col = "steelblue", border = "magenta")

pie(table(dados$`Região de Procedência`),
    labels = round(100 * prop.table(table(dados$`Região de Procedência`))))

hist(x = dados$`Salario (x Sal Min)`,
     xlab = "Salário (x Sal. Mín.)",
     ylab = "Densidade",
     main = "Distribuição da variável Salário",
