...
- Denotamos os quartis por \structure{$Q_1$}, \structure{$Q_2$} e \structure{$Q_3$}:
    - \structure{$Q_1$} é o \structure{primeiro quartil}. É o valor que tem 25% dos dados iguais ou menores do que ele.
    - \structure{$Q_2$} é o \structure{segundo quartil}. É o valor que tem 50% dos dados iguais ou menores do que ele^[Note que o segundo quartil é também a mediana de um conjunto de dados.].
    - \structure{$Q_3$} é o \structure{terceiro quartil}. É o valor que tem 75% dos dados iguais ou menores do que ele.

## Quartis {.allowframebreaks}

- Uma forma de encontrar as posições de \structure{$Q_1$} e \structure{$Q_3$}^[Já aprendemos a encontrar a posição de $Q_2$, a mediana.] na __série ordenada de dados__, como foi visto para o caso da mediana (\structure{$Q_2$}).
    + posição de $Q_1$: \structure{$\frac{n+1}{4}$}
    + posição de $Q_3$: \structure{$\frac{3(n+1)}{4}$}
...
- Note que entre \structure{$Q_1$} e \structure{$Q_3$} estão 50% dos valores mais centrais da distribuição.
...
- Temos que \structure{$Q_1 = 4,9$}, \structure{$Q_3 = 5,25$}, e portanto, a amplitude entre quartis é
